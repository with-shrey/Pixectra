...
public class ImageController {
    ...
    public static void placeOrder(CheckoutData data) {
        SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss_", Locale.getDefault());
        String folder = "Present/" + format.format(new Date());
        Random random = new Random();
        folder = folder + random.nextInt(100) + 1;
        ...
        for (Pair<Product, Vector<Bitmap>> pair : CartHolder.getInstance().getCart()) {
            ...
            for (Bitmap image : pair.second) {
                StorageReference riversRef = mStorageRef.child(pair.first.getId() + "_" + i).child(j + ".png");
                riversRef.putBytes(getBytes(image))
                        .addOnSuccessListener(new OnSuccessListener<UploadTask.TaskSnapshot>() {
                            @Override
                            public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                                Log.v("Image Upload",taskSnapshot.getDownloadUrl().toString());
